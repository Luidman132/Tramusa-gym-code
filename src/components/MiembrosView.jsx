...
import { Search, UserPlus, ChevronRight, ArrowLeft, UserCircle, Phone, Mail, ShieldAlert, CalendarDays, CreditCard, CheckCircle, X, QrCode, Download } from 'lucide-react'
...
export default function MiembrosView({ setVistaActiva, miembroPreSeleccionado, setMiembroPreSeleccionado }) {
  const { miembros, actualizarMiembro, agregarRegistro } = useGym()
  ...
  function confirmarRenovacion() {
    const planInfo = planesRenovacion[renovarPlan]
    const planLabel = planInfo.label.split(' ')[0]

    // Calcular nuevas fechas
    const hoy = new Date()
    const fechaFin = new Date(hoy)
    fechaFin.setMonth(fechaFin.getMonth() + planInfo.meses)

    const cambios = {
      estado: 'activo',
      plan: planLabel,
      inicio: formatFechaDDMMYYYY(hoy),
      fin: formatFechaDDMMYYYY(fechaFin),
    }

    // 1. Actualizar estado global del miembro
    actualizarMiembro(miembroViendo.id, cambios)

    // 2. Registrar el cobro en el historial (para Ingresos Hoy del Dashboard)
    agregarRegistro({
      tipo: 'cobro',
      titulo: `Renovación - ${miembroViendo.nombre}`,
      detalle: `Cobro renovación ${planLabel}: S/ ${renovarMonto}. Cliente: ${miembroViendo.nombre}`,
    ...
    // 4. Mostrar boleta
    setTicketExito({
      cliente: miembroViendo.nombre,
      operacion: `Renovación: ${planLabel}`,
      monto: renovarMonto,
  ...
  function confirmarPase() {
    ...
    setTicketExito({
      cliente: miembroViendo.nombre,
      operacion: `Pase por ${diasPase} día(s)`,
      monto: montoPase,
      fecha: new Date().toLocaleDateString('es-PE')
    })
  }
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{miembroViendo.correo || 'No registrado'}</p>
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm dark:shadow-none border border-slate-200 dark:border-slate-800 flex flex-col">
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-800 flex flex-col gap-3">
              ...
              {miembroViendo.estado === 'vencido' && (
                <>
                  <button
                    onClick={() => { setRenovarMonto(''); setRenovarPlan('mensual'); setMostrarModalRenovacion(true) }}
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden flex flex-col text-center">
            ...
            <div className="p-4 flex justify-between items-center border-b border-slate-100 bg-slate-50">
              ...
              <button onClick={() => setMostrarModalQR(false)} className="text-slate-400 hover:text-slate-600 bg-white rounded-full p-1 shadow-sm cursor-pointer">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-8 flex flex-col items-center justify-center bg-white" ref={qrRef}>
              ...
              <p className="text-xs font-bold text-slate-400 tracking-widest uppercase mb-6">ID: {miembroViendo.codigoQR || 'SIN-CODIGO'}</p>

              {/* El Código QR */}
              <div className="bg-white p-4 rounded-2xl border-2 border-slate-100 shadow-sm inline-block">
                {miembroViendo.codigoQR ? (
                  <QRCode value={miembroViendo.codigoQR} size={180} level="H" />
                ) : (
              ...
              <p className="text-[10px] text-slate-400 mt-4">TRAMUSA S.A. - ES MACHUPICCHU Y SUS COMUNIDADES.</p>
            </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden text-center">
            <div className="bg-emerald-500 p-8 flex flex-col items-center justify-center">
              ...
              <h3 className="text-2xl font-bold text-white">¡Cobro Exitoso!</h3>
            </div>

            <div className="p-8">
              {ticketExito.monto ? (
                <>
                  <p className="text-slate-500 text-sm mb-1">Se ha registrado el pago de:</p>
                  <p className="text-3xl font-black text-slate-800 tracking-tight mb-6">S/ {ticketExito.monto}</p>
                </>
              ) : (
                <p className="text-slate-500 text-sm mb-6">Operación registrada correctamente.</p>
              )}

              <div className="bg-slate-50 rounded-xl p-4 text-left border border-slate-100 mb-8 space-y-2">
                <div className="flex justify-between text-sm"><span className="text-slate-500">Cliente:</span> <span className="font-semibold text-slate-700">{ticketExito.cliente}</span></div>
                <div className="flex justify-between text-sm"><span className="text-slate-500">Concepto:</span> <span className="font-semibold text-slate-700">{ticketExito.operacion}</span></div>
              </div>

              <div className="flex flex-col gap-3">
                <button
                  onClick={() => setMostrarTicketFinal(true)}
                  className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3.5 rounded-xl transition-colors shadow-sm"
                >
                  Ver Ticket / Enviar por WhatsApp
                </button>
                <button
                  onClick={() => setTicketExito(null)}
                  className="w-full bg-slate-100 hover:bg-slate-200 text-slate-600 font-semibold py-3.5 rounded-xl transition-colors"
                >
                  Cerrar y Volver
                </button>
              </div>
            ...
            {miembrosFiltrados.map(m => (
              ...
              >
                ...
                <div className="sm:col-span-1 flex items-center justify-end gap-3">
                  {m.estado === 'activo' && (
                    <span className="inline-flex items-center justify-center px-3 py-1 text-xs font-bold rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 whitespace-nowrap shadow-sm">
                      Activo
                    </span>
                  )}
                  {m.estado === 'vencido' && (
                    <span className="inline-flex items-center justify-center px-3 py-1 text-xs font-bold rounded-full bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-500/20 whitespace-nowrap shadow-sm">
                      Vencido
                    </span>
                  )}
                  {m.estado === 'pase_activo' && (
                    <span className="inline-flex items-center justify-center px-3 py-1 text-xs font-bold rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 whitespace-nowrap shadow-sm">
                      Pase Activo
                    </span>
                  )}
