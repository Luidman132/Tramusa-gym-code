    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
            ...
            {PERIODOS.map(p => (
              <button
                ...
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                  periodo === p.id ? 'bg-white text-slate-800 shadow-sm' : 'text-slate-500 hover:text-slate-700'
                }`}
              >
                {p.label}
              </button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2.5 bg-violet-50 rounded-xl"><DollarSign size={20} className="text-violet-600" /></div>
          ...
          <p className="text-xs text-slate-400 mt-1">{totalCobros} cobros en el periodo</p>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2.5 bg-emerald-50 rounded-xl"><CalendarCheck size={20} className="text-emerald-600" /></div>
            <span className="text-sm text-slate-500">Asistencias</span>
          </div>
          <p className="text-2xl font-bold text-slate-800">{totalAsistencias}</p>
          <p className="text-xs text-slate-400 mt-1">entradas en el periodo</p>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2.5 bg-blue-50 rounded-xl"><TrendingUp size={20} className="text-blue-600" /></div>
            <span className="text-sm text-slate-500">Promedio diario</span>
          </div>
          <p className="text-2xl font-bold text-slate-800">{promedioAsistencias}</p>
          <p className="text-xs text-slate-400 mt-1">asistencias por dia</p>
        </div>
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2.5 bg-amber-50 rounded-xl"><Users size={20} className="text-amber-600" /></div>
            <span className="text-sm text-slate-500">Total Miembros</span>
          </div>
          <p className="text-2xl font-bold text-slate-800">{totalMiembros}</p>
          <p className="text-xs text-slate-400 mt-1">{activos} activos, {vencidos} vencidos</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <h3 className="text-base font-semibold text-slate-800 mb-4">Asistencias por dia</h3>
            ...
            {datosPorDia.map((d, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative">
                  {/* Tooltip */}
                  <div className="absolute bottom-full mb-2 hidden group-hover:block bg-slate-800 text-white text-[10px] px-2 py-1 rounded-lg whitespace-nowrap z-10">
                    {formatFechaCorta(d.fecha)}: {d.asistencias} entradas
                  </div>
                  {d.asistencias > 0 && (
                    <span className="text-[10px] text-slate-500 font-medium">{d.asistencias}</span>
                  )}
                  <div
                    className={`w-full rounded-t-md transition-all ${esHoy ? 'bg-red-500' : 'bg-emerald-400'} group-hover:opacity-80`}
          ...
          <div className="flex items-center gap-4 mt-3 pt-3 border-t border-slate-100">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-slate-400">Dias anteriores</span>
            </div>
            <div className="flex items-center gap-1.5">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <h3 className="text-base font-semibold text-slate-800 mb-4">Ingresos por dia</h3>
          <div className="flex items-end gap-1 h-40">
            {datosPorDia.map((d, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative">
                  ...
                  {d.ingresos > 0 && (
                    <span className="text-[10px] text-slate-500 font-medium">S/{d.ingresos.toFixed(0)}</span>
                  )}
                  <div
                    className={`w-full rounded-t-md transition-all ${esHoy ? 'bg-red-500' : 'bg-violet-400'} group-hover:opacity-80`}
                    style={{ height: `${Math.max(altura, 2)}%`, minHeight: '2px' }}
                  />
                </div>
              )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <h3 className="text-base font-semibold text-slate-800 mb-5">Miembros por estado</h3>
          <div className="flex items-center gap-8">
            ...
            <div className="relative w-36 h-36 shrink-0">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-2xl font-bold text-slate-800">{totalMiembros}</span>
            ...
            <div className="space-y-3 flex-1">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-bold text-slate-800">{activos} <span className="text-slate-400 font-normal">({porcentajeActivos.toFixed(0)}%)</span></span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full bg-blue-500" />
                  <span className="text-sm text-slate-600">Pases activos</span>
                </div>
                <span className="text-sm font-bold text-slate-800">{pases} <span className="text-slate-400 font-normal">({porcentajePases.toFixed(0)}%)</span></span>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-slate-800">{vencidos} <span className="text-slate-400 font-normal">({porcentajeVencidos.toFixed(0)}%)</span></span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <h3 className="text-base font-semibold text-slate-800 mb-5">Miembros por plan</h3>
              ...
              return (
                <div key={plan}>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm font-bold text-slate-800">{cantidad}</span>
                  </div>
          ...
          <div className="border-t border-slate-100 pt-5">
            <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Exportar datos</h4>
            <div className="flex gap-3">
              <button
                onClick={exportarMiembrosCSV}
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium bg-slate-50 border border-slate-200 text-slate-700 hover:bg-slate-100 transition-colors"
              >
                <Download size={15} />
                Miembros CSV
              </button>
